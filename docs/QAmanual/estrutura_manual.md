...
Os testes de API são considerados parte dos testes de integração:

- Implementados com **RestAssured**.  
- Validam:
  - respostas HTTP  
  - estrutura e conteúdo dos payloads  
  - códigos de erro  
  - integração com a base de dados  
- Executados automaticamente em cada commit.

Testes API bem-sucedidos são obrigatórios para aprovação de merge requests.
...
### 4.6 Testes Não Funcionais e Atributos de Arquitetura
