...
(() => {
  ...
  // API functions
  async function searchFacilities({ location, sport, startTime, endTime } = {}) {
  ...
  function createFieldCard(field) {
    ...
    div.innerHTML = `
      ...
      <div class="card-body">
        <div class="field-sport text-uppercase fw-bold text-accent small">${(field.sport||'').toString()}</div>
        <h5 class="field-name card-title fw-bold text-accent-dark">${field.name || field.title || 'Unnamed'}</h5>
        <div class="field-location d-flex align-items-center gap-1 text-muted small">
          <i class="material-icons icon-small">location_on</i> ${field.location || field.city || ''}
        </div>
        <div class="field-details d-flex justify-content-between align-items-center mt-3 pt-3 border-top">
          <div class="field-price fw-bold text-accent-dark">${field.price ? `€${field.price}/hora` : ''}</div>
  ...
  async function handleShowEquipments(facilityId, field = {}) {
    ...
    if (!panel) {
      ...
      Object.assign(panel.style, { right: '20px', top: '80px', width: '320px', zIndex: 1200 });
    ...
    if (list.length === 0) {
      ul.innerHTML = '<div class="text-muted">Nenhum equipamento disponível.</div>';
      return;
    }
    ul.innerHTML = list.map(eq => `
      <div class="d-flex justify-content-between align-items-center py-1">
        <div>
          <div class="fw-bold">${eq.name}</div>
          <small class="text-muted">${eq.description || ''}</small>
        </div>
        <div class="text-end"><small>${eq.price ? `€${eq.price}` : ''}</small></div>
      </div>
    `).join('');
  }
  ...
  async function handleQuickRent(facilityId, field = {}) {
    ...
    const start = prompt('Data/hora início (YYYY-MM-DDTHH:MM), ex: 2025-11-27T19:00');
    if (!start) return;
    const end = prompt('Data/hora fim (YYYY-MM-DDTHH:MM), ex: 2025-11-27T21:00');
    if (!end) return;
    const equipmentIdsRaw = prompt('IDs de equipamentos separados por vírgula (opcional)');
    const equipmentIds = equipmentIdsRaw ? equipmentIdsRaw.split(',').map(s => s.trim()).filter(Boolean).map(Number) : [];
    const payload = {
  ...
  function bindSearch() {
    const searchBtn = document.getElementById('searchBtn') || document.querySelector('.search-bar button') || document.querySelector('.search-btn');
    const input = document.getElementById('searchInput') || document.querySelector('.search-bar input');
    const sport = document.getElementById('sportFilter') || document.querySelector('.search-bar select');
  ...
  document.addEventListener('DOMContentLoaded', () => {
    bindSearch();
    // attempt an initial load (no filters)
    // optional: auto-search on load
    // searchFacilities().then(r => { if (r.ok) renderSearchResults(Array.isArray(r.body)? r.body : (r.body?.results||[]), '#featured'); });
  });
